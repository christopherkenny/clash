...
coc_get_ranking_players <- function(location,
  ...
  # Call to API ---
  resp <- req_base() |>
    httr2::req_url_path_append('locations', location, 'rankings', 'players') |>
    req_header(key) |>
  ...
  out$league <- lapply(out$league, function(x) x |> widen() |> dplyr::bind_rows())
